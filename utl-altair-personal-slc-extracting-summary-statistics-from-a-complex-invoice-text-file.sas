...
156       %utlfkil(d:/xls/orderstatusout.xlsx);
157
158       libname xls excel "d:/xls/orderstatusout.xlsx";
NOTE: Library xls assigned as follows:
      Engine:        OLEDB
      Physical Name: d:/xls/orderstatusout.xlsx

NOTE: Directory contains files of mixed engine types
159       libname sd1  "d:/sd1";
NOTE: Library sd1 assigned as follows:
      Engine:        WPD
      Physical Name: d:\sd1

160
161       data xls.orderstatus;
162         infile "d:/txt/orderstatus.txt";
163         input @',' STATE $4. @@; /*--- last @ hold the buffer ---*/
164         input
165             @'DISCOUNT CODE:' discount_code $4.
166             @'STORE ID:' store_id  6.
167             @'STORE TOTAL-->' store_total @'0909'x sales ;
168       run;
...
NOTE: The infile 'd:\txt\orderstatus.txt' is:
      ...
      File size (bytes)=6480,
      Create Time=13:13:09 Oct 18 2025,
      Last Accessed=13:15:27 Oct 18 2025,
      Last Modified=13:13:57 Oct 18 2025,
      Lrecl=32767, Recfm=V
...
Parse the numeric va;ues creating this excel file


libname xls excel "d:/xls/orderstatusout.xlsx";
libname sd1 sas7bdat "d:/sd1";

data sd1.orderstatus;
  infile "d:/txt/orderstatus.txt";
  input @'DATE:' DATE $10. @@; /*--- last @ hold the buffer ---*/
  input
      @'DISCOUNT CODE:' discount_code $4.
      @'STORE ID:' store_id  6.
      @'STORE TOTAL-->' store_total @'0909'x sales ;
run;quit;
...
I have a very long text file where lines like the following

see
https://community.altair.com/discussion/38109
https://community.altair.com/discussion/38109/altair-monarch-exercise-20-order-status-analysis#latest

PREP

  Download orderstatus.txt from



occur ata random. The can after a dozen lines https://community.altair.com/discussion/38109
...
Contents
   1 What is the Total Sale Amount by Store ID? (input d:/txt/orderstatus.txt)
   2 Total Discounted Sale Amount by Store ID (

Prep Download files into

  1  d:/txt/orderstatus.txt
  2  d:/xls/discountcode.xlsx

Contents
   1 What is the Total Sale Amount by Store ID? (d:/txt/orderstatus.txt)
   2 Total Discounted Sale Amount by Store ID
...
data xls.orderstatus sd1.orderstatus;
  infile "d:/txt/orderstatus.txt" lrecl=120;
  input
      @'DISCOUNT CODE:' discount_code $4.
      @'STORE ID:' store_id  6.
      @'STORE TOTAL-->' store_total 8. @'0909'x  sales;
run;quit;

libname xls clear;

proc print data=orderstatus;
run;quit;
...
2 Total Discounted Sale Amount by Store ID  (d:/xls/discountcode.xlsx)
=========================================================================

libname xlsinp excel "d:/xls/d:/xls/discountcode.xlsx
libname xlsout excel "d:/xls/d:/xls/discountcodeout.xlsx

data xlsout.discountout;
  set xlsinp.sheet1;
...
1   What is the Total Sale Amount by Store ID?
2 Using the DiscountCode spreadsheet, determine the Total Discounted Sale Amount by Store ID
3 Determine How many records contain the word 'BACKORDER'?


libname xls excel "d:/x;s/discountcode.xlsx";

data


             d:/xls/orderstatus.xlsx
...
What is the Total Sale Amount by Store ID?



if index(_infile_,'ID:')

  store=scan(_infile_,2,'ID:');
  sales=scan(_infile_,-1);
...
DENVER, CO  45607
 34755   95900
                11/01/04 11/30/04 9S32052S46 YELLOW   10  100.00  1000.00  SHIPPED FIVE BUT
                                                                           HAD TO BACKORDER
                                                                           FIVE
                11/01/04 11/30/04 9S33042S46 WHITE     3   80.00   240.00  SHIPPED 2 -
                                                                           BACKORDER 2
                11/01/04 11/30/04 9S33042S46 BLACK     2  115.00   230.00  SHIPPED
                11/01/04 11/30/04 9S34068S46 BLACK     3  112.00   336.00  SHIPPED 5 BUT
...
STONEHAM, MA 02180
          28382   33902
                         09/01/04 10/30/04 9S3010A    VIOLET    3  108.00   324.00  SHIPPED
                         09/01/04 10/30/04 9S3011Q    PURPLE    5   93.00   465.00  SHIPPED 3
          34633   6539
                         11/01/04 11/30/04 9S31009S10 YELLOW    4   80.00   320.00  SHIPPED 3
                         11/01/04 11/30/04 9S33079S10 SILVER    8   98.00   784.00  CANCELED PENDING
                                                                                    BILL REVIEW
          34633   6539
                         11/01/04 11/30/04 9S33079S10 BLUE      8   98.00   784.00  SHIPMENT LOST
...
WILMINGTON, MA  01887
          34564   98342
         11/30/04 11/30/04 9S31158S11 BLACK     2   54.00   108.00  SHIPPED
         11/01/04 11/30/04 9S39017S33 SILVER    2  115.00   230.00  SHIPMENT DAMAGED -
                                                                    REORDERED
...
data _null_;
  ...
  put _infile_;
  input;
  set sd1.orderstatus   point=_n_ nobs=numobs;
  put @6 DISCOUNT_CODE @18 STORE_ID @28 STORE_TOTAL @38 SALES;
...
--------------------+---+
| A1| fx| DISCOUNNT_CODE |
----------------------------------------------
[_] |    A     |    B    |    C    |   DE    |
----------------------------------------------
    | DISCOUNT | STORE   | STORE   |         |
 1  | CODE     |  ID     | TOTAL   | SALEST  |
 -- |----------+---------+---------+---------+
 2  |C         | 23      | 46      | 5184    |
 -- |----------+---------+---------+---------+
 3  |A         | 7       | 202     | 20387   |
